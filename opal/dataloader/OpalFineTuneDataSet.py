...
class OpalFinetuneDataset(Dataset):
    ...
    def _json_to_natural_format(self, json_obj):
        ...
        if isinstance(json_obj, dict):
            if "steps" in json_obj:
                # New simplified format with direct steps
    ...
    def _format_troubleshoot_response(self, response):
        """Format simplified troubleshoot responses in a natural way."""
        output = []
        
        # Process steps directly (no more action/execution wrapper)
